...
public class VoxelTilePlacerWfc : MonoBehaviour
{
    ...
    private float placeTileCooldown = 0.05f;
    private float placeTileTimer = 0f;
    private Queue<Vector2Int> tilesToPlace = new Queue<Vector2Int>();
    ...
    private void Generate()
    {
        ...
        {
            tilesToPlace.Enqueue(new Vector2Int(x, y));
        }
        isPlacingTiles = true;
        placeTileTimer = 0f;

        shouldBuildNavMesh = true;
        navMeshBuildDelay = 5f;
    }

    private void Update()
    {
        if (isPlacingTiles && tilesToPlace.Count > 0)
        {
            placeTileTimer += Time.deltaTime;
            if (placeTileTimer >= placeTileCooldown)
            {
                placeTileTimer = 0f;
                Vector2Int pos = tilesToPlace.Dequeue();
        ...
        {
            navMeshBuildDelay -= Time.deltaTime;
            if (navMeshBuildDelay <= 0f)
            {
                shouldBuildNavMesh = false;
